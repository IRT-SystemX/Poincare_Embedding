...
from function_tools import euclidean_function, poincare_module, gmm_tools
...
class EuclideanEmbedding(nn.Module):
    def __init__(self, n_exemple, cuda=False, lr=1e-2, verbose=True, negative_distribution=None,
        ...
        self.cuda = cuda
        self.N = n_exemple
        self.W = nn.Embedding(n_exemple, 2)
        if(self.cuda):
            self.W.cuda()
        ...
        for i in progress_bar:
            ...
            for example, neigbhors, walks in dataloader:
                ...
                if(gamma > 0):
                    ...
                    loss_o3 = (-pi_z.detach() * torch.log(1e-4 + gmm_tools.weighted_gmm_pdf(pi_z.detach(), r_example, mu.detach(), sigma.detach(), self.d, norm_factor=1)))

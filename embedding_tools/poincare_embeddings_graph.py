...
class RiemannianEmbedding(nn.Module):
    ...
    def fit_softmax(self, dataloader, alpha=1.0, beta=1.0, gamma=0.0, pi=None, mu=None, sigma=None, max_iter=100,
            negative_sampling=5):
        ...
        for i in progress_bar:
            ...
            for example, neigbhors, walks in dataloader:
                ...
                loss_o1 = self.log_softmax(-self.d(me,mw)).mean()
                ...
                loss_o2 = torch.log(
                    1 + (torch.exp(-(negative_d - positive_d.unsqueeze(-1).expand_as(negative_d)))).sum(-1)).mean()

                #loss_o2 = self.log_softmax() +

                loss = alpha * loss_o1 + beta * loss_o2
                if (gamma > 0):
                    r_example = self.W(example).squeeze()
                    pi_z = pi[example].squeeze()
                    loss_o3 = (-pi_z.detach() * torch.log(
                        1e-4 + gmm_tools.weighted_gmm_pdf(pi_z.detach(), r_example, mu.detach(), sigma.detach(),
                                                          self.d)))
                    # print("loss o3 size ->", loss_o3)
                    loss += gamma * loss_o3.mean()
                    loss_value3 = loss_o3.sum(-1).mean().item()
            ...
            if (self.verbose):
                progress_bar.set_postfix(
                    {"O1": alpha * loss_value1, "O2": beta * loss_value2, "O3": gamma * loss_value3, "PDF": loss_pdf3})